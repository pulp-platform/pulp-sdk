...
void layerConvBNRelu6(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_lambda.loc = (uint32_t) l1_buffer + 35344;
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 18440) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  ...
  for(iter=0; iter<2*1*2*16; iter++) {
    ...
    if(iter<2*1*2*16-1) 
    {
      ...
      W_tile_size_nof = (_i_nof_load+1 == 2) ? 64 : 64;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 24, 2, 128, 32, 128,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 64, 1*1, 128, 1*1, 128, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 18440) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+16896 + 256*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 35344 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int32_t *) (l1_buffer + 34828 + exec_db_act);
    lambda = (int32_t *) (l1_buffer + 35344 + exec_db_act);
    W = (char *) (l1_buffer + 18440 + exec_db_W);
    ...
    x_tile_size_h_exec   = (_i_h_exec+1 == 2)   ? 8 : 24;
    x_tile_size_w_exec   = (_i_w_exec+1 == 16)   ? 2 : 2;
    y_tile_size_nof = (_i_nof_exec+1 == 2) ? 64 : 64;
    y_tile_size_h   = (_i_h_exec+1 == 2)   ? 8 : 24;
    y_tile_size_w   = (_i_w_exec+1 == 16)   ? 2 : 2;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
    y_length_nof_byte = (_i_nof_exec+1 == 2)   ? 64 : 64;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    if(iter<2*1*2*16-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 24, 2, 64, 32, 128, 0, 0, 0, 0, 0, 0, 8), // ext
