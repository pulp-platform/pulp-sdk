 ...
 *
 * ricevo due eventi 10, legati al cmd, uno vicino all'altro ma non esegue handler
 *================================================================================================**/
 ...
 **                                         GLOBAL VARIABLE
 *================================================================================================**/
static struct i2c_itf_data_s *g_i2c_itf_data[ARCHI_UDMA_NB_I2C] = {0};
...
static void __pi_i2c_tx_handler(int event, void *arg);
static void __pi_i2c_rx_handler(int event, void *arg);
static void __pi_i2c_eot_handler(int event, void *arg);
static void __pi_i2c_cmd_handler(int event, void *arg);
...
static inline void __pi_i2c_enqueue_tx(uint32_t device_id, uint32_t l2buf, uint32_t size);
static inline void __pi_i2c_enqueue_rx(uint32_t device_id, uint32_t l2buf, uint32_t size);
static inline void __pi_i2c_enqueue_cmd(uint32_t device_id, uint32_t l2buf, uint32_t size);
...
int pi_i2c_detect(struct pi_device *device, struct pi_i2c_conf *conf, uint8_t *rx_data)
{
	...
	int32_t res = __pi_i2c_detect(cs_data, conf, rx_data, &task_block);
	pi_task_wait_on(&task_block);
	pi_task_destroy(&task_block);
