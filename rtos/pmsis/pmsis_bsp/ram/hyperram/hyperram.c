...
int hyperram_ioctl(struct pi_device *device, uint32_t cmd, void *arg)
{
  switch (cmd)
  {
    case PI_RAM_IOCTL_ID:
    {
      hyperram_t *hyperram = (hyperram_t *)device->data;
      pi_hyper_ioctl(&hyperram->hyper_device, PI_HYPER_IOCTL_SET_TRAN_ID, arg);
      break;
    }
  }
  return 0;
...
int hyperram_id_alloc(struct pi_device *device)
{
  hyperram_t *hyperram = (hyperram_t *)device->data;
  
  return pi_hyper_id_alloc(&hyperram->hyper_device);    
}

int hyperram_ctl_status_regs(struct pi_device *device, uint32_t cmd, void *arg)
{
  hyperram_t *hyperram = (hyperram_t *)device->data;
  return pi_hyper_ctl_status_regs(&hyperram->hyper_device, cmd, arg);
}
