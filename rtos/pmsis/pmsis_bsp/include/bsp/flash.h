...
static inline void pi_flash_bwrite(struct pi_device *device,
  uint32_t pi_flash_addr, void *buffer, int nb_word);
...
typedef struct __pi_flash_api_t {
  ...
  void (*bwrite)(struct pi_device *device, uint32_t pi_flash_addr, void *buffer, int nb_word);
