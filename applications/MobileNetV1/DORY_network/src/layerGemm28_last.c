...
void layerGemm28_last(
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 1112) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  20480, // size: dimension of matrix of weight * bytes_per_weight
  1024, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
  1024, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
  20, // length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  1024, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  &dma_evt // copy
  );
  ...
  for(iter=0; iter<1*1*1*1; iter++) {
    ...
    db_W = !db_state_W ? 20480 : 0;
    db_y = !db_state_y ? 80 : 0;
    exec_db_x = 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 20480 : 0;
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 20, 1*1, 1024, 1*1, 1024, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 1112) + db_W, // loc
    ...
    W = (char *) (l1_buffer + 1112 + exec_db_W);
    ...
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 20 : 20;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 1)   ? 80 : 80;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      ...
      mchan_barrier(dma_evt);
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 1, 1, 20, 1, 1000, 0, 0, 0, 0, 0, 0, 32), // ext
