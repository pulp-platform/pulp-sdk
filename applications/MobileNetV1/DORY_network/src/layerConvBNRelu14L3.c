...
void __attribute__ ((noinline)) layerConvBNRelu14L3(void *args) 
{
  ...
  if(pi_core_id()==0)
  {
    ...
    pi_cl_ram_read(hyperram, l3_W+262144, transfer_weights + 65536, 512, &buff_req_w2);
    pi_cl_ram_read(hyperram, l3_W+264192, transfer_weights + 65536 + 512, 512, &buff_req_w3);
  ...
  for(int k=0; k<4; k++) 
  {
    if (k < 3) 
    {
      if(pi_core_id()==0) 
      {
        ...
        pi_cl_ram_read(hyperram, l3_W+262144+ (k+1)*512, transfer_weights + 65536, 512, &buff_req_w2);
        pi_cl_ram_read(hyperram, l3_W+264192 + (k+1)*512, transfer_weights + 65536+ 512, 512, &buff_req_w3);
      }
    }  
