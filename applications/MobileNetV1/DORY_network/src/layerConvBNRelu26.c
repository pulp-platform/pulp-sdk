...
void layerConvBNRelu26(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 16;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+4112;
    copy_lambda.loc = (uint32_t) l1_buffer + 20576;
  ...
  for(iter=0; iter<1*1*1*1; iter++) {
    ...
    db_act = !db_state_W ? 16 : 0;
    exec_db_x = 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 4096 : 0;
    exec_db_act = db_state_W ? 16 : 0;
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+4096 + 16*_i_nof_load;
          ...
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+4112 + 16*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 20576 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int32_t *) (l1_buffer + 20556 + exec_db_act);
    lambda = (int32_t *) (l1_buffer + 20576 + exec_db_act);
