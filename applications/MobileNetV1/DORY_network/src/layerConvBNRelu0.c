...
void layerConvBNRelu0(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_lambda.loc = (uint32_t) l1_buffer + 36470;
  ...
  for(iter=0; iter<1*1*2*8; iter++) {
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+864 + 128*_i_nof_load;
          ...
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+992 + 128*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 36470 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int32_t *) (l1_buffer + 36210 + exec_db_act);
    lambda = (int32_t *) (l1_buffer + 36470 + exec_db_act);
