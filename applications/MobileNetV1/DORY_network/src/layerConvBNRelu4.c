...
void layerConvBNRelu4(
  ...
  for(iter=0; iter<1*1*2*16; iter++) {
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 24, 2, 64, 32, 64,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+8192 + 512*_i_nof_load;
          ...
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+8704 + 512*_i_nof_load;
    ...
    lambda = (int32_t *) (l1_buffer + 35856 + exec_db_act);
    ...
    if(iter<1*1*2*16-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 24, 2, 128, 32, 128, 0, 0, 0, 0, 0, 0, 8), // ext
