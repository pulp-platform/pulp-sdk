...
void layerConvDWBNRelu3(
  ...
  for(iter=0; iter<2*3*1; iter++) {
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+576 + 128*_i_nof_load;
          ...
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+832 + 128*_i_nof_load;
    ...
    k = (int32_t *) (l1_buffer + 35404 + exec_db_act);
    lambda = (int32_t *) (l1_buffer + 35664 + exec_db_act);
