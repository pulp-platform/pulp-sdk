...
void layerConvDWBNRelu23(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.merge = 0;
    copy_k.size = (uint16_t) 768;
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+4608;
    copy_k.loc = (uint32_t) l1_buffer + 34188;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 768;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+6656;
    copy_lambda.loc = (uint32_t) l1_buffer + 35728;
  ...
  192, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  &dma_evt // copy
  );
  ...
  1728, // size: dimension of matrix of weight * bytes_per_weight
  ...
  192, // length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  ...
  for(iter=0; iter<3*1*1; iter++) {
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (_i_nif_load+1 == 3) ? 128 : 192;
      ...
      x_length_nif_byte = (_i_nif_load+1 == 3)   ? 128 : 192;
      ...
      W_tile_size_nof = (_i_nof_load+1 == 3) ? 128 : 192;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom_hwc_to_chw(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 8, 192, 8, 512,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      {
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 192.0, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 8), // ext
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+4608 + 768*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 34188 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 4;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+6656 + 768*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 35728 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 3)   ? 128 : 192;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    {  
      ...
      {                                       
        ...
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 4, 4, 192, 4, 512, 0, 0, 0, 0, 0, 0, 8), // ext
