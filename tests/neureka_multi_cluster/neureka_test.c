...
static void pe_entry(void *arg) {
  int id = pi_core_id(); //extract the core id of each core in the cluster
  if(id == 0) {
    glob_errors = run_test();//core 0 configures neureka
  } 
  else//while core0 configures neureka other cores perform some other operations
  {
    initialize_mat(id);//matrix initialization
    add_mat(id);//matrix addition
  }
  pi_cl_team_barrier();
}
