...
datamover_status_t datamover_cim_layout(uint8_t *matrix_in, uint8_t *matrix_out, uint32_t size_m, uint32_t size_n, uint32_t row_tile_size) {
  // For row-major to CIM A-layout: row_tile_size = CIM inner dimension [elements] (64)
  // For row-major to CIM B-layout: row_tile_size = CIM outer dimension [elements] (8x #CIM)
  int acq_to = 1000000;
  int job_id = -1;
...
datamover_status_t datamover_unfold(uint8_t *matrix_in, uint8_t *matrix_out, uint32_t size_c, uint32_t dim_h, uint32_t dim_w) {
  ...
  // dim_h, dim_w: dimensions of the input tensor
  ...
  uint32_t w_tiles = (dim_w + DATAMOVER_BANDWIDTH_ELEMS - 1) / DATAMOVER_BANDWIDTH_ELEMS;  // number of tiles in W dimension (rounded up)
  ...
  datamover_in_set((uint32_t)matrix_in);
  datamover_out_set((uint32_t)matrix_out);
  datamover_tot_len_set(c_tiles * w_tiles * DATAMOVER_BANDWIDTH_ELEMS * dim_h);
  datamover_in_d0_set(dim_h * dim_w, c_tiles * DATAMOVER_BANDWIDTH_ELEMS);
  datamover_in_d1_set(DATAMOVER_BANDWIDTH_ELEMS, w_tiles);
  datamover_in_d2_set(dim_w, dim_h);
  datamover_in_d3_set(0, 0);
  datamover_out_d0_set(size_c * dim_h * dim_w / P, side_P);
  ...
  datamover_out_d3_set(size_c * dim_h * dim_w / side_P, side_P);
  datamover_in_out_d4_stride_set(size_c * dim_w / side_P, 0);
  datamover_matrix_dim_set(dim_w, dim_h);
  datamover_channels_set(size_c * dim_h * dim_w, size_c);
...
datamover_status_t datamover_unfold_blocking(uint8_t *matrix_in, uint8_t *matrix_out, uint32_t size_c, uint32_t dim_h, uint32_t dim_w, uint64_t timeout) {
  datamover_status_t status;
  status = datamover_unfold(matrix_in, matrix_out, size_c, dim_h, dim_w);
  if (status != DATAMOVER_OK) {
    return status;
  }
  status = datamover_wait_done(timeout);
...
datamover_status_t datamover_fold(uint8_t *matrix_in, uint8_t *matrix_out, uint32_t size_c, uint32_t dim_h, uint32_t dim_w) {
  ...
  // dim_h, dim_w: dimensions of the folded (OUTPUT) tensor
  const int P = 4;      // Patch size (number of elements in a patch), only tested for P=4 (2x2)
  const int side_P = 2; // Patch sidelength
  int acq_to = 1000000;
  int job_id = -1;
  ...
  datamover_out_set((uint32_t)matrix_out);
  datamover_tot_len_set(c_tiles * w_tiles * DATAMOVER_BANDWIDTH_ELEMS * dim_h);
  datamover_in_d0_set(size_c * dim_h * dim_w / P, side_P);
  datamover_in_d1_set(size_c, (w_tiles * DATAMOVER_BANDWIDTH_ELEMS) / side_P);
  datamover_in_d2_set(DATAMOVER_BANDWIDTH_ELEMS, c_tiles);
  datamover_in_d3_set(size_c * dim_h * dim_w / side_P, side_P);
  datamover_out_d0_set(dim_h * dim_w, c_tiles * DATAMOVER_BANDWIDTH_ELEMS);
  datamover_out_d1_set(DATAMOVER_BANDWIDTH_ELEMS, w_tiles);
  datamover_out_d2_set(dim_w, dim_h);
  datamover_out_d3_set(0, 0);
  datamover_in_out_d4_stride_set(0, size_c * dim_w / side_P);
  datamover_matrix_dim_set(dim_w, dim_h);
  datamover_channels_set(size_c * dim_h * dim_w, size_c);
...
datamover_status_t datamover_fold_blocking(uint8_t *matrix_in, uint8_t *matrix_out, uint32_t size_c, uint32_t dim_h, uint32_t dim_w, uint64_t timeout) {
  datamover_status_t status;
  status = datamover_fold(matrix_in, matrix_out, size_c, dim_h, dim_w);
  if (status != DATAMOVER_OK) {
    return status;
  }
  status = datamover_wait_done(timeout);
