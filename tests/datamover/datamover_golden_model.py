...
def main():
    ...
    if args.datamover_mode==0:
        ...
        t = args.transp_mode
        if t not in [1, 2, 4]:
            raise ValueError(f"Unsupported TRANSP_MODE: {t}. Supported modes: 1, 2, 4.")
        C, M, N = input_tensor.shape
        assert N % t == 0, f"SIZE_N ({N}) must be divisible by transp_mode ({t})"
        output_tensor = input_tensor.reshape(C, M, N // t, t).transpose(0, 2, 1, 3).reshape(C, N // t, M * t)
    ...
    # print("Input Tensor:")
    # print(input_tensor)
    # print("\nGolden Output Tensor:")
    # print(output_tensor)

    print("\nSuccessfully generated golden model output tensor.")
