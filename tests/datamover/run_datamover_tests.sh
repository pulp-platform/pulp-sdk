...
# builds and runs datamover test for each configuration.
# Usage: run from tests/datamover/:
#   ./run_datamover_tests.sh [--test <name>] [--resume <name>] [--trace <path>]
...
#   --trace <path>    enable GVSOC trace for the given component path (e.g. datamover)
  ...
  case "$1" in
    --test|--resume|--trace)
      if [[ $# -lt 2 ]]; then
        ...
        echo "Usage: $0 [--test <name>] [--resume <name>] [--trace <path>]"
        exit 1
      fi
      case "$1" in
        --test)   FILTER_TEST="$2" ;;
      ...
      shift 2 ;;
    *) echo "Unknown argument: $1"; echo "Usage: $0 [--test <name>] [--resume <name>] [--trace <path>]"; exit 1 ;;
  esac
done
...
for entry in "${TESTS[@]}"; do
  ...
  python3 "$GOLDEN_MODEL" $gm_args --output_filename "$GENERATED_HEADER"
  ...
  run_args=""
  [[ -n "$TRACE_PATH" ]] && run_args="runner_args=--trace=$TRACE_PATH"
  # shellcheck disable=SC2086
  if ! riscv make -C "$SCRIPT_DIR" run $run_args 2>&1 | tee "$log_file"; then
