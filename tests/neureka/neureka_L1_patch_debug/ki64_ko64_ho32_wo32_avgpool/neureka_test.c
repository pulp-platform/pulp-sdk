...
int run_test() {
  ...
  volatile uint8_t* x        = ne16_infeat;
  volatile uint8_t* nq       = ne16_scale;
  volatile uint8_t* nqs      = ne16_scale_shift;
  volatile uint8_t* nqb      = ne16_scale_bias;
  volatile uint8_t* golden_y = ne16_outfeat;
  volatile uint8_t* actual_y = ne16_streamin;
  volatile uint8_t* W        = ne16_weights;
  ...
  printf("Data from infeat\n");
  printf("Data from infeat=%x\n",x);
  ...
  for(int i=6; i<24; i++) {
    NEUREKA_WRITE_REG(i*4, ne16_cfg[i]);
  }
  ...
  int errors = neureka_compare_int(actual_y, golden_y, 10);
  return errors;
}
