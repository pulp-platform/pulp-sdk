...
void Rnnacc_v1::setup_streamer_matmul_x() {
    ...
    printf("n_w_o_tiles, %d, %d, %d", n_w_o_tiles, this->n_output, bandwidth_words );


    if(this->n_w_o_rest > 0){
        printf("judihui, %d, %d", n_w_o_rest, n_w_o_tiles);
        this->n_w_o_tiles += 1;
        printf("judihui, %d, %d", n_w_o_rest, n_w_o_tiles);
    } 

    // int stride_inp = min(2*bandwidth_words, 2*this->n_output);
    int stride_inp = 2*bandwidth_words;
    // int stride_out = min(2*bandwidth_words, 2*this->n_output);
    // int stride_out = min(2*bandwidth_words, 2*this->n_output_external);
    // int stride_out = min(2*bandwidth_words, 2*this->n_output_external);
    int stride_out = 2*this->n_output_external;
    // int stride_out = 0;
    // if(bandwidth_words > this->n_output_external) {
    //     stride_out = 2*this->n_output_external*this->n_w_o_tiles;
    // } else if (n_w_o_rest>0){
    //     // stride_out = min(2*bandwidth_words, 2*this->n_output_external); //2*this->n_output_external;
    //     stride_out = 2*this->n_output_external; //2*this->n_output_external;
    // } else {
    //     stride_out = min(2*bandwidth_words, 2*this->n_output_external)*this->n_w_o_tiles;
    // }

    // this->n_w_o_rest  = this->n_output; // % (this->NR_MASTER_PORTS); 
    // this->n_w_o_tiles = this->n_output; //int(this->n_output / (this->NR_MASTER_PORTS)); 
    ...
    this->vl_w = RnnaccVectorLoad<uint8_t>(
        ...
        stride_inp, // 2*bandwidth_words,  // d0_stride
        this->n_w_o_tiles, // d0_length
        stride_out, // 2*bandwidth_words,   // d1_stride
    ...
    //     stride_inp, // 2*bandwidth_words,  // d0_stride
    //     this->n_w_o_tiles, // d0_length
    //     stride_out*this->n_w_o_tiles, // 2*bandwidth_words,   // d1_stride
    //     this->n_w_i_tiles, // d1_length
    //     0,                 // d2_stride
    //     true               // debug
    // );
...
//             this->w_i_idx = 0;
//         }
//     } else if(this->w_i_idx < this->n_w_i_tiles-1) {
//       this->w_i_idx++;
//     }
}
...
int Rnnacc_v1::perform_matmul_x() {
    ...
    // for (auto i=this->w_o_idx*this->NR_MASTER_PORTS; i<(this->w_o_idx+1)*this->NR_MASTER_PORTS; i+=4) {
    for (auto i=this->w_o_idx*2*this->NR_MASTER_PORTS; i<(this->w_o_idx+1)*2*this->NR_MASTER_PORTS; i+=4) {
        ...
        // Accumulate buf_accum reg with all the multipications
        // xt::view(acc0, 1) = xt::view(mult0, 1) + xt::view(mult1, 1);
        // xt::view(acc1, 1) = xt::view(mult2, 1) + xt::view(mult3, 1);

        // xt::view(acc2, 1) = xt::view(acc0, 1) + xt::view(acc1, 1);
        ...
        //     }
        //         // std::cout << "[MULT] w_o_idx="<< std::dec << std::setw(16) << w_o_idx<< std::dec << std::endl;
        // }

        // write back to buf_accum regs
        // xt::view(this->buf_accum, w_o_idx, 1) = xt::cast<int32_t>(xt::view(acc3, 1));
        // xt::view(this->buf_accum, i, 1) = xt::cast<int32_t>(xt::view(acc3, 1));
