...
class Cluster(object):
  ...
  def __init__(self, tp, cluster_id):
    ...
    ext2loc_converter_ports    = 32
    ext2loc_ts_converter_ports = 1
    ...
    cluster.l1_ico.ext2loc = Component(properties=OrderedDict([
      ...
      ('output_width', 4),
      ('output_align', 4),
      ('nb_master_ports', ext2loc_converter_ports)
    ]))
    ...
    cluster.l1_ico.ext2loc_ts = Component(properties=OrderedDict([
      ...
      ('output_width', 4),
      ('output_align', 4),
      ('nb_master_ports', ext2loc_ts_converter_ports)
    ]))
    ...
    for i in range(0, ext2loc_converter_ports):
        cluster.l1_ico.ext2loc.set('out_%d' % i, cluster.l1_ico.interleaver.new_itf('in_%d' % (nb_pe + i)))
    for i in range(0, ext2loc_ts_converter_ports):
        cluster.l1_ico.ext2loc_ts.set('out_%d' % i, cluster.l1_ico.interleaver.new_itf('ts_in_%d' % (nb_pe + i)))
